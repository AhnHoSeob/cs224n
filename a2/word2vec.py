...
def naiveSoftmaxLossAndGradient(
    ...
    loss = -np.log(softmax(np.matmul(outsideVectors, centerWordVec))[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + sum(softmax(np.matmul(outsideVectors,centerWordVec)).reshape(-1,1) * outsideVectors) 
    
    gradOutsideVecs = softmax(np.matmul(outsideVectors,centerWordVec)).reshape(-1,1) * centerWordVec    #loss에 모든 vw가 들어가 있기 때문에 모든 vw에 대해 gradient를 구해야 한다
    gradOutsideVecs[outsideWordIdx] -= centerWordVec    #outside vector에 대한 gradient는 내적값을 포함
...
def negSamplingLossAndGradient(
    ...
    negSampleWordVecs = outsideVectors[indices]
    dot = np.matmul(negSampleWordVecs,centerWordVec)    # (K+1,) , index 0 is outside vector
    
    

    loss = -np.log(sigmoid(dot[0])) - sum(np.log(sigmoid(-dot[1:])))

    gradCenterVec = - (1-sigmoid(dot[0])) * negSampleWordVecs[0] + sum((1-sigmoid(-dot[1:]).reshape(-1,1)) * negSampleWordVecs[1:])
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -(1-sigmoid(dot[0])) * centerWordVec
    for idx, neg_idx in enumerate(indices[1:]):
        gradOutsideVecs[neg_idx] += (1-sigmoid(-dot[idx+1])) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss_single, gradCenter,gradOutside = word2vecLossAndGradient(centerWordVec,outsideWordIndex,outsideVectors,dataset)
        loss += loss_single
        gradCenterVecs[centerWordIndex] += gradCenter
