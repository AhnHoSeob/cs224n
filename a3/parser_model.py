...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        # x = torch.tensor([torch.index_select(self.embeddings,0,w[i]).view(-1) for i in range(len(w))]) 
        # x = torch.stack(x)
    ...
    def forward(self, w):
        ...
        hidden_layer = torch.matmul(x,self.embed_to_hidden_weight) + self.embed_to_hidden_bias
